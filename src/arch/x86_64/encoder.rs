...
pub fn encode_instruction(dest: &mut context::Context, ins: &isa::Ins) {
    ...
    match &ins {
        ...
        Ins::IMul(ops) => match (&ops.dest, &ops.src) {
            (Operand::Register(left), Operand::U32(n)) => {
                rex_prefix(dest, ops, true, false);
                if *n <= 0xFF {
                    dest.code.push(0x6B);
                    modrm(dest, ops.dest.clone(), ops.dest.clone(), 0);
                    dest.code.push(*n as u8);
                } else {
                    dest.code.push(0x69);
                    modrm(dest, ops.dest.clone(), ops.dest.clone(), 0);
        ...
        Ins::IDiv(_) => unimplemented!(),
        Ins::Cmp { ops, .. } => match (&ops.dest, &ops.src) {
            (Operand::Register(_), Operand::U32(n)) |
            (Operand::Memory { .. }, Operand::U32(n)) => {
                rex_prefix(dest, ops, true, false);
                if *n <= 0xFF {
                    dest.code.push(0x83);
                } else {
                ...
                match &ops.dest {
                    Operand::Memory { disp, base } => {
                        if 0 <= *disp && *disp <= 127 {
                            dest.code.push(*disp as u8);
                        } else if -127 <= *disp && *disp < 0 {
...
fn modrm(dest: &mut context::Context, odest: Operand, osrc: Operand, extension: u8) {
    match (odest, osrc) {
        ...
        (Operand::Memory { disp, base }, right) if right.is_lit() => dest
            .code
            .push(if -127 <= disp && disp <= 127 {
                0b01_000_000
            } else {
                0b10_000_000
            } | (extension << 3) | modrm_normalize_reg(base as u8)),
